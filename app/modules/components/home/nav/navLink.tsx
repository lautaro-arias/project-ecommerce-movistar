...
export const NavLinkMovil = () => {
    ...
    return (
        <>
            {pathname !== '/cart' ?
                <ul tabIndex={0} className="bg-gray-100  border border-gray-200 shadow-lg menu menu-sm dropdown-content mt-3 z-[1] p-2  rounded-box w-52">
                    {itemNav.map((item, index) => (
                        <Link href={item.url} key={index} className=' bg-white btn btn-ghost text-md border border-gray-200 mt-4 text-gray-600 font-light  cursor-pointer '>
                            {item.name}
                        </Link>

                    ))}
                </ul>
                : <ul tabIndex={0} className="bg-gray-100 menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow  rounded-box w-52">
                    {itemNav.map((item, index) => (
                        <a onClick={() => { handleClickRemoveProductNav(0) }} href={item.url} key={index} className=" bg-white btn btn-ghost text-md border border-gray-200 mt-4 text-gray-600 font-light  cursor-pointer ">
                            {item.name}
                        </a>
...
export const NavLinkCart = () => {
    ...
    return (
        <div className="indicator bg-white  cursor-pointer">
            <Link href='/cart' >

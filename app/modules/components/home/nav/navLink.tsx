...
export const NavLinkMovil = () => {
    ...
    return (
        <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
            {itemNav.map((item, index) => (
                <Link key={index} className=' text-lg mt-4 text-gray-600 font-light border-b border-cyan-500 cursor-pointer hover:border-b ms-8 me-8 hover:border-cyan-400 hover:text-cyan-500' href={item.url}>{item.name}</Link>
